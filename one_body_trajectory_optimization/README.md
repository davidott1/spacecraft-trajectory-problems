...
| **Constraints** <br> **Equality** <br> <br> **Inequality** <br> <br> | $\Theta(t)$ <br> $\Psi(t)$ | <br> Initial : $t_o=t_{os} \ \ \ \underline{r}(t_0)=r_{os} \ \ \ v(t_0)=v_{os}$ <br> Final : $t_f=t_{fs} \ \ \ r(t_f)=r_{fs} \ \ \ v(t_f)=v_{fs}$ <br> min fuel : $\Gamma(t) \le \Gamma_{\max}$ or $T\le T_{\max}$ <br> min energy : $\underline{\Gamma}(t)\le \Gamma_{\max}$ or $T\le T_{\max}$ or unconstrained |
