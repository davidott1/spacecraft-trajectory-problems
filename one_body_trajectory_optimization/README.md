...
```math
\begin{array}{ll}
& \text{smin}(a_1, a_2, k) = \frac{-1}{k} (m + \ln(e^{-k a_1 - m} + e^{-k a_2 - m})) & \text{where} & m = \max(k a_1, k a_2) \\
& \text{smax}(a_1, a_2, k) = \frac{ 1}{k} (m + \ln(e^{ k a_1 - m} + e^{ k a_2 - m})) & \text{where} & m = \max(k a_1, k a_2)
\end{array}
```
