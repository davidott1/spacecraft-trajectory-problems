...
```
The time-derivative of the state $\dot{\vec{x}}$ must conform to the dynamics, so `x_vec_dot = f(x_vec,u_vec)`. Substituting the dynamics into the Hamilitonian yields
```math
