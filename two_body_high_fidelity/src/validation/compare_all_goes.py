#!/usr/bin/env python3
"""
Compare TLE vs HORIZONS ephemeris for all GOES satellites (16, 18, 19)
"""
...
def compare_satellite(norad_id, sat_name):
    ...
    print(f"\nError statistics for {sat_name.upper()}:")
...
def plot_comparison(results):
    """
    Create comparison plots for all satellites
    ...
    # Create multi-satellite comparison plot
    ...
    for idx, res in enumerate(results):
        ...
        ax.set_title(f'{sat_name.upper()} - X Component')
        ax.legend()
        ...
        ax.set_title(f'{sat_name.upper()} - Position Error (mean={res["error_mean"]:.1f} km)')
        ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('all_goes_comparison.png', dpi=150)
    print(f"\nComparison plot saved to: all_goes_comparison.png")
    ...
    ax.set_xlabel('Hours from Oct 1, 2025 00:00 UTC')
    ax.set_ylabel('Position Error (km)')
    ax.set_title('TLE vs HORIZONS Position Error - All GOES Satellites')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('all_goes_error_summary.png', dpi=150)
    print(f"Error summary plot saved to: all_goes_error_summary.png")


def main():
    """Main function to compare all GOES satellites"""
    results = []
    ...
    if results:
        ...
        print("="*80)
        for res in results:
            print(f"{res['sat_name'].upper():10s} - Mean error: {res['error_mean']:6.1f} km  "
                  f"(min: {res['error_min']:6.1f}, max: {res['error_max']:6.1f}, "
                  f"std: {res['error_std']:6.1f} km)")
