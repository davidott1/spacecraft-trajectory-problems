...
Run with:
  pytest src/validation/test_dynamics.py -v
"""
import pytest
import numpy as np

from src.model.dynamics  import TwoBodyGravity, Gravity, AtmosphericDrag, Acceleration
...
class TestTwoBodyGravity:
  ...
  def test_point_mass_leo_magnitude(self):
    """Point mass acceleration at LEO altitude should be ~9 m/s²."""
    gravity = TwoBodyGravity(gp=SOLARSYSTEMCONSTANTS.EARTH.GP)
  ...
  def test_j2_acceleration_not_zero(self):
  ...
  def test_j2_smaller_than_point_mass(self):
...
class TestAccelerationCoordinator:
  ...
  def test_drag_adds_to_gravity(self):
