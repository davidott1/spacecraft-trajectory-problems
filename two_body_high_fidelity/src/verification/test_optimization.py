...
class TestPatchedConicFunctions:
  ...
  def test_circular_velocity(self):
    ...
    """
    pos_mag = SOLARSYSTEMCONSTANTS.EARTH.RADIUS.EQUATOR + 200_000.0  # 200 km altitude
  ...
  def test_hohmann_estimates(self):
    ...
    """
    radius_o = SOLARSYSTEMCONSTANTS.EARTH.RADIUS.EQUATOR + 200_000.0  # 200 km LEO
    radius_f = SOLARSYSTEMCONSTANTS.MOON.SMA                          # Moon orbit
    gp       = SOLARSYSTEMCONSTANTS.EARTH.GP
  ...
  def test_two_body_propagation_quarter(self):
    ...
    # After quarter period, position should be at (0, pos_mag, 0)
  ...
  def test_two_body_propagation_energy_conservation(self):
    ...
    pos_mag_f = np.linalg.norm(state_f[0:3])
    vel_mag_f = np.linalg.norm(state_f[3:6])
    E_f = 0.5 * vel_mag_f**2 - gp / pos_mag_f
...
class TestLunarTransfer:
  ...
  def test_evaluate_transfer_reaches_soi(self):
    ...
    max_pos_mag = np.max(np.linalg.norm(result['trajectory_states'][0:3, :], axis=0))
    assert max_pos_mag > 1e8, f"Max distance = {max_pos_mag/1000:.0f} km, expected > 100,000 km"
